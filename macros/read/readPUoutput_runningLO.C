...
#include "TLegend.h"
#include "TString.h"
#include "../include/samplesPDFNLO.h"
#include <vector>

using namespace std;

void readPUoutput_runningLO() {
  ...
  if(jmass==0) file_mass0="mt160";
  if(jmass==1) file_mass0="mt161";
  if(jmass==2) file_mass0="mt162";
  if(jmass==3) file_mass0="mt163";
  if(jmass==4) file_mass0="mt164";
  if(jmass==5) file_mass0="mt165";
  if(jmass==6) file_mass0="mt166";
  if(jmass==7) file_mass0="mt167";
  if(jmass==8) file_mass0="mt168";
  if(jmass==9) file_mass0="mt169";
  if(jmass==10) file_mass0="mt170";


  for(int kmass=0; kmass<3; kmass++) {

    if(kmass==0) file_mass=file_mass0;
    if(kmass==1) file_mass=file_mass0+"_plus05";
    if(kmass==2) file_mass=file_mass0+"_minus05";
    ...
    for(int k=kmin; k<kmax; k++) {
      ...
      /*      if(file_mass=="Mt170"){
	if(file_pdf=="CT1090cl_") alphasNLO_NEW=0.109698; //for mt=170, mu=1
	...
	if(file_pdf=="CT1090cl_") alphasNLO_NEW=0.109474; //for mt=172.5, mu=1
	...
	if(file_pdf=="NNPDF_") alphasNLO_NEW=0.108499;
      ...
      else file_prefix="../../LOmtpole/";
      TString outputfile="../../rootfiles/"+file_pdf+"_"+file_energy+"_"+file_mass+"_"+file_scale+"_"+file_kt+"_LO.root";

      ofstream fout ("../../logfiles/"+file_pdf+"_"+file_energy+"_"+file_mass+"_"+file_scale+"_"+file_kt+"_LO.out");

      cout<<"output file: "<<outputfile<<endl;
      cout<<"output log:  "<<"../../logfiles/"+file_pdf+"_"+file_energy+"_"+file_mass+"_"+file_scale+"_"+file_kt+"_LO.out"<<endl;
      std::vector<TString> file_string_;
      file_string_=samples_LO(file_mass, file_pdf, file_scale);
      ...
      fout<<"# "<<endl;
      fout<<"######################################################################################################### "<<endl;
      fout<<"# ROOT FILE  "<<file_pdf+file_energy+"_"+file_mass+"_"+file_scale+"_"+file_kt+"_LO.root"<<endl;
      fout<<"######################################################################################################### "<<endl;
      fout<<"# "<<endl;
      fout<<"# "<<endl;
	...
	fout<<"# "<<endl;
      ...
      TString topmass( file_mass(2,3) );
      TString topmass1(file_mass(6,file_mass.Sizeof()-4));
      TString deltatopmass;
      if(topmass1=="plus05") deltatopmass="0.5";
      if(topmass1=="minus05") deltatopmass="-0.5";
      ...
      fout<< "# DELTATOPPOLEMASS = "<<deltatopmass<<endl;
      fout<< "# SCALEFACTOR = "<<scalevalue<<endl;
      fout<< "# SCALE = SCALEFACTOR*TOPPOLEMASS, no SCALEFACTOR*(TOPPOLEMASS+DELTATOPMASS)"<<endl;
